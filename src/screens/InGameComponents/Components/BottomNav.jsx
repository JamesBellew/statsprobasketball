...
const BottomNav = ({
  ...
  // --- TIMER MODAL: Render for both mobile and desktop/tablet ---
  const timerModal = showTimeModal && (
  ...
  if (isMobile) {
    // Mobile layout: stack rows, sticky to bottom
    return (
      <>
        {timerModal}
        <div className="fixed bottom-0 left-0 w-full z-50 bg-primary-bg shadow-lg px-2 pb-2 flex flex-col space-y-2">
          {/* Row 1: Previous/Next Period */}
          <div className="flex w-full space-x-2 ">
            <button
              disabled={currentQuater === 1}
              onClick={handlePreviousPeriodClick}
              className={`flex-1 h-12 bg-secondary-bg rounded-lg flex items-center justify-center text-white text-lg ${currentQuater === 1 ? "line-through bg-secondary-bg/50 text-gray-400" : ""}`}
            >
            ...
            <button
              ...
              className={`flex-1 h-12 bg-secondary-bg rounded-lg flex items-center justify-center text-white text-lg ${currentQuater === 4 ? "bg-secondary-bg text-gray-200" : ""} ${currentQuater === 8 ? "line-through text-gray-500" : ""}`}
            >
              Next Period
              <FontAwesomeIcon className="ml-2" icon={faForward} />
            </button>
          </div>
          {/* Row 2: Always show Lineout, and show Timer if minutesTracked */}
          <div className="flex w-full space-x-2 items-center justify-center">
            {/* Team Lineout (always show) */}
            {passedLineout && (
              <div
                className="flex items-center justify-center flex-[1_1_0%] bg-secondary-bg h-12 rounded-md cursor-pointer"
            ...
            {minutesTracked && (
              <div className="flex flex-[2_1_0%] items-center justify-center space-x-2 bg-secondary-bg h-12 rounded-md cursor-pointer" onClick={() => setShowTimeModal(true)}>
                <span className={`text-2xl ${isRunning ? "text-primary-cta" : "text-gray-400"}`}>{minutes}</span>
                <span className="text-2xl">:</span>
                <span className={`text-2xl ${isRunning ? "text-primary-cta" : "text-gray-400"}`}>{seconds < 10 ? `0${seconds}` : seconds}</span>
                <button className="ml-2" onClick={e => { e.stopPropagation(); setIsRunning(!isRunning); }}>
